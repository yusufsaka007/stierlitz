#include "data_handler.hpp"

int recv_buf(int __fd, char* __buf, int __len) {
    int rc;

    rc = recv(__fd, __buf, __len, 0);
    if (rc < 0) {
        return RECV_ERROR;
    } else if (rc == 0) {
        return PEER_DISCONNECTED_ERROR; // Connection closed
    }
    __buf[rc] = '\0';
}

#ifdef SERVER
int recv_buf(int __fd, char* __buf, int __len, const std::atomic<bool>& __shutdown_flag) {
    int rc;
    int epoll_fd = epoll_create1(0);
    if (epoll_fd == -1) {
        return EPOLL_CREATE_ERROR;
    }

    epoll_event ev, events[1];
    ev.events = EPOLLIN;
    ev.data.fd = __fd;

    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, __fd, &ev) == -1) {
        rc = EPOLL_CTL_ERROR;
        goto recv_buf_error;
    }

    while (!__shutdown_flag.load()) {
        int nfds = epoll_wait(epoll_fd, events, MAX_EVENTS, 500);
        if (nfds == -1) {
            rc = EPOLL_WAIT_ERROR;
            break;
        } else if (nfds == 0) {
            continue; // Timeout, no events
        }

        if (events[0].data.fd == __fd) {
            ssize_t bytes_received = recv(client_fd, __buf, __len, 0);
            if (bytes_received < 0) {
                rc = RECV_ERROR;
                break;
            } else if (bytes_received == 0) {
                close(epoll_fd);
                return 0;
            } else {
                __buf[bytes_received] = '\0';
                close(epoll_fd);
                return bytes_received;
            }
        }
    }
    recv_buf_error:
        close(epoll_fd);
        return rc;

}

int send_buf(int __fd, const char* __buf, int __len=-1, const std::atomic<bool>& __shutdown_flag) {
    ssize_t bytes_sent;
    ssize_t total_bytes;

    if (__len == -1) {
        __len = strlen(__buf);
    }

    while (!shutdown_flag.load() && total_bytes < __len) {
        bytes_sent = send(__fd, __buf + total_bytes, __len - total_bytes, 0);
        if (bytes_sent < 0) {
            return SEND_ERROR;
        } else if (bytes_sent == 0) {
            return PEER_DISCONNECTED_ERROR; // Connection closed
        }

        total_bytes += bytes_sent;
    }

    return total_bytes;
}

#endif // SERVER